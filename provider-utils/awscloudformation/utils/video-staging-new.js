...
function build(context, resourceName, projectType, props) {
  ...
  buildCustomLambda(context, props);
}
...
function buildCustomLambda(context, props) {
  const { amplify } = context;
  const targetDir = amplify.pathManager.getBackendDirPath();
  const customDir = `${targetDir}/video/${props.shared.resourceName}/custom/LambdaFunctions`;
...
function syncHelperCF(context, props, projectType) {
  ...
  if (!fs.existsSync(`${targetDir}/video/${props.shared.resourceName}/build/`)) {
    fs.mkdirSync(`${targetDir}/video/${props.shared.resourceName}/build/`);
  }
  ...
  fs.copySync(`${pluginDir}/cloudformation-templates/${serviceMetadata.stackFolder}/`, `${targetDir}/video/${props.shared.resourceName}/build/`, { filter: filterForEJS });
}
...
async function copyFilesToS3(context, options, resourceName, projectType) {
  ...
  const buildDirPath = `${targetDir}/video/${resourceName}/build/`;
  const customDirPath = `${targetDir}/video/${resourceName}/custom/`;
      ...
      foldersToZip.forEach((lambdaName) => {
        ...
        if (fs.existsSync(`${customDirPath}/${lambdaName}`)) {
          promiseFilesToUpload.push(
            zipLambdaFunctionsAndPush(context, lambdaName, `${customDirPath}/${lambdaName}`,
              customDirPath, s3Client, targetBucket, stackFolder),
          );
        } else {
          promiseFilesToUpload.push(
            zipLambdaFunctionsAndPush(context, lambdaName, `${buildDirPath}/${lambdaName}`,
